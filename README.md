...
- `predql_tasks/`: Contains the core logic for task definition and generation.
  - `base/`: Base classes for defining tasks (`PredQLBaseTask`, `PredQLStatTask`, `PredQLTmpTask`).
  - `tasks/`: Implementations of specific static and temporal tasks.
- `experiments/`: Contains Jupyter notebooks and scripts for model training and task generation.
  - `model_training/`: Scripts and notebooks for training GNNs like SAGE and HGT, including configuration files and utility functions.
  - `task_generation/`: Notebooks demonstrating how to generate tasks over relational databases.
...
- **`PredQLBaseTask`**: A simplified version of the `BaseTask` class from *RelBench*.   
...
- **`PredQLStatTask`**: A specialized class for defining static deep learning tasks.
- **`PredQLTmpTask`**: A specialized class for defining temporal deep learning tasks.

For specific examples of task implementations, check the `predql_tasks/tasks/` directory.
...
- **GraphSAGE** (`sage_model.py`)
- **Heterogeneous Graph Transformer (HGT)** (`hgt_model.py`)
...
Detailed training configurations can be found in the config file: `experiments/model_training/notebooks/config.yml`.

### 📔 Notebooks

The `experiments/task_generation/relational_fel.ipynb` notebook is used to analyze datasets and showcase tables from newly generated tasks.

The `experiments/model_training/notebooks/` directory contains various Jupyter notebooks for testing different task types:
- Temporal Binary Classification (`_bc.ipynb`)
- Temporal Multiclass Classification (`_mcc.ipynb`)
- Temporal Multilabel Classification (`_mlc.ipynb`)
- Temporal Regression (`_reg.ipynb`)
